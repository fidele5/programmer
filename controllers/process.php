...
class Process {
    ...
    public function __construct(array $datas, array $suggered) {
        ...
        $this->datas = $this->datasAdapter($datas);
    ...
    private function datasAdapter(array $datas)
    {
        $target = array(
            "prepa" => array(),
            "G1" => array(),
        ...
        foreach($datas as $promotion => $cours)
        {
            foreach($cours as $cour)
            {
                $target[$promotion][] = $cour["id"];
            }        
        }
        return $target;
    }
